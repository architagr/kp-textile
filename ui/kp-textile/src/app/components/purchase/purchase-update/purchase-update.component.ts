...
export class PurchaseUpdateComponent implements OnInit {
  ...
  getPurchaseOrderData() {
    this.showSpinnerCount++;
    this.purchaseService.getPurchaseOrder(this.billNo).subscribe({
      next: (data) => {
        for (let index = 1; index < data.data.bailDetails.length; index++){
          this.addBail();
        } 
        this.addPurchaseForm.patchValue(data.data)
        this.showSpinnerCount--;
      },
      complete: () => {
  ...
  ngOnInit(): void {
    this.billNo = this.route.snapshot.paramMap.get('purchaseBillNumber') ?? ''
  ...
  submitData() {
    ...
    this.purchaseService.updatePurchase(data).subscribe({
      next: (data) => {
        this.toastr.info('<span class="tim-icons icon-bell-55" [data-notify]="icon"></span> Purchase order updated.', 'Success', {
