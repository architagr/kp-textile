...
@Component({
  ...
  styleUrls: ['./sales-update.component.scss']
})
export class SalesUpdateComponent implements OnInit {
  ...
  getBailsDetails(index: number): BailDetailsDto[]{
    ...
    let data =  this.overAllBailInfo[qualityId]
    if (data.length === 0){
      data.push({
        bailNo:this.getBailControl(index)['bailNo'].value,
        billNo:"",
        billedQuantity:0,
        pendingQuantity:0,
      } as BailDetailsDto)
  ...
  ngOnInit(): void {
    this.billNo = this.route.snapshot.paramMap.get('salesBillNumber') ?? ''
    this.getAllCodes();
    this.getAllQuality();
    this.getAllTransporter();
    this.getSalesBillData();
  }
  getSalesBillData(){
    this.showSpinnerCount++;
    this.salesService.getSales(this.billNo).subscribe({
      next:(response)=>{
        for (let index = 1; index < response.data.bailDetails.length; index++){
          this.addBail();
        } 
        this.addPurchaseForm.patchValue(response.data)
        this.showSpinnerCount--;
      }
    })
  }
