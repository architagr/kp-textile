...
type ITransporterPersistance interface {
	GetPersonByTransporterId(request commonModels.GetTransporterRequestDto) ([]commonModels.TransporterContactPersonDto, *commonModels.ErrorDetail)
	GetTransporter(request commonModels.GetTransporterRequestDto) (commonModels.TransporterDto, *commonModels.ErrorDetail)
	GetTransporterTotalByFilter(filterData commonModels.TransporterListRequest) (int64, *commonModels.ErrorDetail)
	GetTransporterByFilter(filterData commonModels.TransporterListRequest) ([]commonModels.TransporterDto, map[string]*dynamodb.AttributeValue, *commonModels.ErrorDetail)
	UpsertTransporter(transporter commonModels.TransporterDto, isNew bool) (*commonModels.TransporterDto, *commonModels.ErrorDetail)
	UpsertTransporterContact(transporterContact commonModels.TransporterContactPersonDto) (*commonModels.TransporterContactPersonDto, *commonModels.ErrorDetail)
	DeleteTransporterContact(branchId, transporterId, contactId string) *commonModels.ErrorDetail
	DeleteTransporter(branchId, transporterId string) *commonModels.ErrorDetail
}
...
func (repo *TransporterPersistance) GetTransporterTotalByFilter(filterData commonModels.TransporterListRequest) (int64, *commonModels.ErrorDetail) {
	var count int64
	proj := expression.NamesList(expression.Name("transporterId"))
...
func (repo *TransporterPersistance) GetTransporter(request commonModels.GetTransporterRequestDto) (commonModels.TransporterDto, *commonModels.ErrorDetail) {
	...
	keyCondition := expression.KeyAnd(
		expression.Key("transporterId").Equal(expression.Value(request.TransporterId)),
		expression.Key("sortKey").Equal(expression.Value(common.GetTransporterSortKey())),
	)
...
func (repo *TransporterPersistance) GetPersonByTransporterId(request commonModels.GetTransporterRequestDto) ([]commonModels.TransporterContactPersonDto, *commonModels.ErrorDetail) {
	keyCondition := expression.KeyAnd(
		expression.Key("transporterId").Equal(expression.Value(request.TransporterId)),
		expression.Key("sortKey").BeginsWith(common.GetTransporterContactSortKey("")),
	)
...
func (repo *TransporterPersistance) UpsertTransporterContact(transporterContact commonModels.TransporterContactPersonDto) (*commonModels.TransporterContactPersonDto, *commonModels.ErrorDetail) {
	fmt.Println("upserting contact - ", transporterContact)
	...
	if err != nil {
		...
		return nil, &commonModels.ErrorDetail{
			ErrorCode:    commonModels.ErrorInsert,
			ErrorMessage: fmt.Sprintf("Error in upserting transporter contact (%s) for transporter id %s, error message; %s", transporterContact.FirstName, transporterContact.TransporterId, err.Error()),
		}
	}
	return &transporterContact, nil
}

func (repo *TransporterPersistance) DeleteTransporterContact(transporterId, contactId string) *commonModels.ErrorDetail {
	_, err := repo.db.DeleteItem(&dynamodb.DeleteItemInput{
		TableName: &repo.transporterTableName,
		Key: map[string]*dynamodb.AttributeValue{
			"transporterId": {
				S: aws.String(transporterId),
			},
			"sortKey": {
				S: aws.String(common.GetTransporterContactSortKey(contactId)),
			},
		},
	})
	if err != nil {
...
func (repo *TransporterPersistance) DeleteTransporter(transporterId string) *commonModels.ErrorDetail {
	_, err := repo.db.DeleteItem(&dynamodb.DeleteItemInput{
		TableName: &repo.transporterTableName,
		Key: map[string]*dynamodb.AttributeValue{
			"transporterId": {
			...
			"sortKey": {
				S: aws.String(common.GetTransporterSortKey()),
			},
		},
	})
	if err != nil {
...
func buildFilterExpression(filterData commonModels.TransporterListRequest, projection *expression.ProjectionBuilder) (*expression.Expression, *commonModels.ErrorDetail) {
	...
	filter := expression.Name("sortKey").BeginsWith(common.TransporterSortKey)
