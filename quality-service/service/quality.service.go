...
func (service *QualityService) Add(quality commonModels.QualityDto) commonModels.QualityResponse {
	qualityService, err := service.qualityRepo.Add(quality)

	if err != nil {
		return commonModels.QualityResponse{
			CommonResponse: commonModels.CommonResponse{
				StatusCode:   http.StatusBadRequest,
				ErrorMessage: fmt.Sprintf("could not add Quality - %+v", quality),
				Errors: []commonModels.ErrorDetail{
					*err,
				},
			},
...
func (service *QualityService) AddMultiple(qualities []commonModels.QualityDto) commonModels.QualityListResponse {
	allCodes, err := service.qualityRepo.AddMultiple(qualities)

	if err != nil {

		if len(allCodes) > 0 && len(qualities) > len(allCodes) {
			return commonModels.QualityListResponse{
				CommonListResponse: commonModels.CommonListResponse{
					CommonResponse: commonModels.CommonResponse{
